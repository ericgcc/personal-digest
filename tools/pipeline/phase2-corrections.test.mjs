...
import { deriveRecoveryFrame, projectEvidence } from "./v2.mjs";
import { ENFORCEABLE_CONSTRAINTS, STYLE_PROFILES, CANONICAL_STYLES } from "./style-profiles.mjs";
...
test("C5: a two-attempt stage reports both calls", async (t) => {
  ...
  const { STAGES_V2 } = await import("./v2.mjs");
  const { RUNS_DIRECTORY } = await import("../lib/shared.mjs");
...
test("C5: the run summary aggregates attempts and prices each call in its own band", async () => {
  const { buildRunSummary } = await import("../lib/shared.mjs");
...
test("C6: the moved rationale is preserved in the architecture documentation", async () => {
  ...
  const rationale = await readFile(path.join(ROOT, "docs", "style-pipeline-rationale.md"), "utf8");
  for (const phrase of [
...
test("C7: the review contract is assembled into the evaluation stage's delivered context", async () => {
  const { assembleStageContext } = await import("./v2.mjs");
  for (const stage of ["developmental-review", "reader-review"]) {
