...
//
// Usage:
//   node scripts/verify-run.mjs --run <run-id> [--digest <digest-id>] [--strict]
//
// Exit codes:
...
// retired v1 pipeline keeps only its static descriptor (`config/pipeline-v1-stages.json`)
// for describing historical runs, loaded here once at startup.
const v1Specs = JSON.parse(
  await readFile(path.join(ROOT, "config", "pipeline-v1-stages.json"), "utf8"),
);
const PIPELINE_STAGES = {
  "editorial-pipeline-v1": v1Specs.map(({ name, artifact }) => [name, artifact]),
  "editorial-pipeline-v2": STAGES_V2.map((stage) => [stage.name, stage.artifact]),
};
...
if (!runId) {
  console.error("Usage: node scripts/verify-run.mjs --run <run-id> [--digest <digest-id>] [--strict]");
  console.error("       node scripts/verify-run.mjs --all [--digest <digest-id>]");
  process.exit(1);
}
