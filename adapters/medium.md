...
This adapter supports source-group `acquisition_filters.exclude_topics` before browser opening.
...
- When `exclude_topics` is configured, exclude a candidate **before opening Chrome** only when the email-visible metadata makes the excluded topic a high-confidence match.
...
- If the metadata is genuinely ambiguous, keep the candidate and follow the normal required reading method. Prefer an occasional unnecessary read over silently excluding a potentially valuable non-matching article.
- A candidate excluded by this filter is `excluded-before-read`: it is not a reviewed source, does not contribute to reading-time savings, and does not appear in the editorial source catalog. Keep an ephemeral count/list for run notes when useful.
