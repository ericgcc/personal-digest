...
// ---------------------
// Before profiles, `STAGES_V2` in `tools/pipeline/v2.mjs` named one union of every
...
import { ROOT, RunnerError, exists } from "../lib/shared.mjs";
...
// declaration in one place. `tools/pipeline/editorial-validation.mjs` consumes them and reads
...
function buildProfile({ id, version, style, label, status, stages, rendering, enforced = [], frameFailurePolicy = "recovery-frame", notes = [] }) {
  ...
  if (!budget) throw new RunnerError(`Style profile ${id} names style ${style}, which has no entry in tools/pipeline/budgets.mjs`);
  ...
  return deepFreeze({
    ...
    budget_source: "tools/pipeline/budgets.mjs",
...
function synthesisMaxV1() {
  ...
  return buildProfile({
    ...
    // in `tools/pipeline/editorial-validation.mjs`; the thresholds are read from
    // `COMPOSITION_BY_STYLE` above, and the body budget from `tools/pipeline/budgets.mjs`.
